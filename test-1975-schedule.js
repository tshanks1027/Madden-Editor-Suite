/**
 * Test script for 1975 schedule application (26 teams, 14-game season)
 * Missing teams: TB(5), JAX(16), CAR(20), BAL(24), SEA(27), HOU(31)
 */
async function test() {
  ...
  const dst = 'C:/Users/tshan/Documents/Madden NFL 26/Saves/CAREER-1975TEST';
  ...
  // Load 1975 schedule
  const schedule = JSON.parse(fs.readFileSync('data/retro/schedules/1975.json', 'utf8'));
  ...
  console.log('\n=== TEAM GAME COUNTS ===');
  const expansionTeams = [5, 16, 20, 24, 27, 31]; // TB, JAX, CAR, BAL, SEA, HOU
  for (let ti = 0; ti < 32; ti++) {
    ...
    const expected = expansionTeams.includes(ti) ? 0 : 14;
    if (count !== expected || marker) {
      console.log(`  Team ${ti} (${name}): ${count} games${marker}`);
    }
  }

  console.log('\n=== TRY LOADING CAREER-1975TEST IN MADDEN ===');
}
