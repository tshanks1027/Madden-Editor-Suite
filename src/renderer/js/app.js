...
class MaddenEditorApp {
    ...
    renderRoster() {
        ...
        this.hotTable = new Handsontable(hotContainer, {
            ...
            // Handle row header clicks to open player card
            afterOnCellMouseDown: (event, coords, td) => {
                // Check if clicked on row header (coords.col === -1)
                // or if double-clicked anywhere on the row
                if (coords.row >= 0 && coords.col === -1) {
                    // Single click on row number opens player card
                    const paginatedIndex = coords.row;
                    const filteredIndex = this.paginatedPlayerIndices[paginatedIndex];
                    const player = this.filteredPlayers[filteredIndex];
    ...
    openPlayerCard(playerData) {
        ...
        // Player info section
        document.getElementById('playerCardAge').textContent = playerData.PAGE || '--';
        document.getElementById('playerCardYearsPro').textContent = playerData.PYRP !== undefined ? playerData.PYRP : '--';
        document.getElementById('playerCardHeight').textContent = this.formatHeight(playerData.PHGT);
        document.getElementById('playerCardWeight').textContent = playerData.PWGT ? `${playerData.PWGT + 160} lbs` : '--';
        document.getElementById('playerCardCollege').textContent = college;
        document.getElementById('playerCardJersey').textContent = playerData.PJEN || '--';
    ...
    populatePlayerRatings(playerData, position) {
        ...
        ratings.forEach(fieldCode => {
            ...
            ratingItem.innerHTML = `
                <div class="rating-label">${fieldDef.shortDisplay || fieldDef.display}</div>
                <div class="rating-value ${ratingClass}">${value}</div>
            `;
