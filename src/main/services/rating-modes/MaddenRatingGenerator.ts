...
export class MaddenRatingGenerator implements IRatingGenerator {
  ...
  private convertRookieStatsToRatings(stats: RookieStats): PlayerRatings {
    ...
    // Map each property explicitly (ALL rating attributes)
    if (stats.povr !== undefined) ratings.POVR = stats.povr;
    ...
    if (stats.ptrk !== undefined) ratings.PTRK = stats.ptrk;

    // Additional rating attributes
    if (stats.pcod !== undefined) ratings.PCOD = stats.pcod;
    if (stats.pbcv !== undefined) ratings.PBCV = stats.pbcv;
    if (stats.pstf !== undefined) ratings.PSTF = stats.pstf;
    if (stats.pspm !== undefined) ratings.PSPM = stats.pspm;
    if (stats.pjum !== undefined) ratings.PJUM = stats.pjum;
    if (stats.pibl !== undefined) ratings.PIBL = stats.pibl;
    if (stats.prbp !== undefined) ratings.PRBP = stats.prbp;
    if (stats.prbf !== undefined) ratings.PRBF = stats.prbf;
    if (stats.ppbp !== undefined) ratings.PPBP = stats.ppbp;
    if (stats.ppbf !== undefined) ratings.PPBF = stats.ppbf;
    if (stats.pldb !== undefined) ratings.PLDB = stats.pldb;
    if (stats.pbrs !== undefined) ratings.PBRS = stats.pbrs;
    if (stats.ptup !== undefined) ratings.PTUP = stats.ptup;
    if (stats.ppwm !== undefined) ratings.PPWM = stats.ppwm;
    if (stats.pfnm !== undefined) ratings.PFNM = stats.pfnm;
    if (stats.pbsh !== undefined) ratings.PBSH = stats.pbsh;
    if (stats.ppur !== undefined) ratings.PPUR = stats.ppur;
    if (stats.pprc !== undefined) ratings.PPRC = stats.pprc;
    if (stats.pmcv !== undefined) ratings.PMCV = stats.pmcv;
    if (stats.pzcv !== undefined) ratings.PZCV = stats.pzcv;
    if (stats.pspc !== undefined) ratings.PSPC = stats.pspc;
    if (stats.pcit !== undefined) ratings.PCIT = stats.pcit;
    if (stats.psrr !== undefined) ratings.PSRR = stats.psrr;
    if (stats.pmrr !== undefined) ratings.PMRR = stats.pmrr;
    if (stats.pdrr !== undefined) ratings.PDRR = stats.pdrr;
    if (stats.phtp !== undefined) ratings.PHTP = stats.phtp;
    if (stats.pprs !== undefined) ratings.PPRS = stats.pprs;
    if (stats.prel !== undefined) ratings.PREL = stats.prel;
    ...
    // Use POVR if available from CSV, otherwise calculate
    if (!ratings.POVR || ratings.POVR === 0) {
      ratings.POVR = this.calculateOVRFromAttributes(ratings, stats.position);
    }
  ...
  /**
   * Calculate OVR from attributes if not provided
   */
  private calculateOVRFromAttributes(ratings: Partial<PlayerRatings>, position: string): number {
