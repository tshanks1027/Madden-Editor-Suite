...
async function saveRosterFile(filePath, players, originalData, options = {}) {
  ...
  try {
    ...
    // This is necessary because equipment changes are queued separately
    const equipmentUpdated = applyPendingEquipmentChanges(file);
    if (equipmentUpdated > 0) {
      console.log(`[RosterParser] Applied ${equipmentUpdated} pending equipment changes`);
    }
...
// Empty equipment options - full list is in HEAD if needed
